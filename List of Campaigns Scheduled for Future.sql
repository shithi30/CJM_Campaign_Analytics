/*
- Viz: https://docs.google.com/spreadsheets/d/1cfP1Y4fuSiCymqrwsmlWRqYz6jGhZSgH1mpPvF1XO5g/edit#gid=655064865
- Data: 
- Function: 
- Table:
- Instructions:
...
- Notes (if any): 
	...
	select * from public.notification_bulknotificationrequest where id=13388; 					-- create
	select * from public.notification_bulknotificationsendrequest where request_id=13388; 		-- schedule
	select * from public.notification_bulknotificationschedule where request_id=13388; 			-- periodic schedule
*/
...
begin  
	...
	truncate table data_vajapora.future_schedules; 
	...
	from   
		...
		where status in(1) -- just 'scheduled'
	...
	loop
		-- scheduled on dates
		insert into data_vajapora.future_schedules
		select 
		...
		var_date:=var_date+1; 
		if var_date='2022-03-31'::date+1 then exit; 
		end if; 
	end loop; 
end $$; 

select *
from data_vajapora.future_schedules
where 
	campaign_id like '%-%'
