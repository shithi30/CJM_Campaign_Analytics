...
begin
	...
	loop
		...
		select 
			*, 
			case 
				when notification_id in(select id from public.notification_pushmessage where concat(title, ' ', summary) like '%${%') then 1
				else 0
			end if_personalized
		from 
		...
		select tbl1.mobile_no, tbl1.if_personalized, tbl3.id, tbl4.if_transacted
		from 
		...
		select
			...
			(select count(distinct mobile_no) from data_vajapora.help_a where event_name='inbox_message_open' and if_personalized=1) personalized_inbox_opened_merchants,
			count(distinct case when if_personalized=1 then mobile_no else null end) open_through_personalized_inbox_merchants, 
			count(distinct case when id is not null and if_personalized=1 then mobile_no else null end) first_open_through_personalized_inbox_merchants, 
			count(distinct case when id is not null and if_personalized=1 and if_transacted=1 then mobile_no else null end) first_open_through_personalized_inbox_merchants_txn, 
			count(distinct case when id is not null and if_personalized=1 and if_transacted is null then mobile_no else null end) first_open_through_personalized_inbox_merchants_nontxn, 
			
			(select count(distinct mobile_no) from data_vajapora.help_a where event_name='inbox_message_open' and if_personalized=0) cjm_inbox_opened_merchants,
			count(distinct case when if_personalized=0 then mobile_no else null end) open_through_cjm_inbox_merchants, 
			count(distinct case when id is not null and if_personalized=0 then mobile_no else null end) first_open_through_cjm_inbox_merchants, 
			count(distinct case when id is not null and if_personalized=0 and if_transacted=1 then mobile_no else null end) first_open_through_cjm_inbox_merchants_txn, 
			count(distinct case when id is not null and if_personalized=0 and if_transacted is null then mobile_no else null end) first_open_through_cjm_inbox_merchants_nontxn, 
